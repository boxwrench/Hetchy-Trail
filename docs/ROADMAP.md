...
| P2 | Minigame framework + The Heading | Module contract, The Heading module, band verification | `minigame_check` + `minigame_sim` + harness | **Done, except arcade mode** — contract and module shipped, all five bands pass. Arcade waits on a Main scene (`scenes/main/` is empty); its default config is decided and recorded. The press-your-luck claim was **withdrawn on measurement** — see [01-the-heading.md](superpowers/specs/minigames/01-the-heading.md) finding 4 |
